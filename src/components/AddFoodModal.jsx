  ...
  const handleSubmit = (e) => {
    ...
    if (showModal?.update) {
      axios(`${process.env.REACT_APP_BASE_URL}/food/${showModal?.data?._id}`, {
        ...
        data: {
          ...formstate,
          ingredients: foodIngredients?.map((data) => {
            return data?._id;
          }),
          image,
        },
  ...
  return (
    <>
      <div
        ...
        className={
          showModal?.show
            ? "fixed top-0 left-0 right-0 z-50  w-full p-4 overflow-x-hidden bg-black/50 overflow-y-auto h-screen opacity-100 duration-500 flex items-center justify-center"
            : " flex items-center justify-centerfixed top-0 left-0 right-0 z-50  w-full p-4 overflow-x-hidden bg-black/50 overflow-y-auto h-screen opacity-0 pointer-events-none duration-500"
        }
      >
        <div className="relative w-[70%] max-h-full overflow-y-scroll m-auto">
          <form
            onSubmit={handleSubmit}
            className="relative rounded-lg shadow bg-darkGray"
          >
            <div className="flex items-start justify-between p-4 border-b rounded-t border-textGray ">
              ...
              <button
                ...
                type="button"
                className="text-white bg-transparent hover:bg-gray-600 duration-200 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center "
              >
                <svg
                  className="w-3 h-3"
                  aria-hidden="true"
            ...
            <div className="flex items-center justify-center pb-6  rounded-b ">
              <Button
                text={showModal?.update ? "Update Food" : "Add Food"}
                className={"w-1/6"}
              />
            </div>
          </form>
